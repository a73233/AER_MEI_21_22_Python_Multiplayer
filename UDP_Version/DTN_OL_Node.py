...
class DTN_OL_Node:
	...
	def createRtpSocketForNextNode(self):
		...
		self.rtpSocket = socket.socket(socket.AF_INET6, socket.SOCK_DGRAM)
	...
	def createRtspSocketForNextNode(self):
		...
		self.sendRtspSocket = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
		try:
	...
	def createRtspSocketForClient(self):
		...
		rtspSocket = socket.socket(socket.AF_INET6, socket.SOCK_STREAM)
	...
	def processRtspRequest(self, data, clientinfo):
		...
		if requestType == self.SETUP:
			...
			try:
				ind = self.clientInfoArr.index(clientinfo)
				self.clientInfoArr[ind]['rtpSocket'] = socket.socket(socket.AF_INET6, socket.SOCK_DGRAM)
			except ValueError as e:
				print("\t\t*********FATAL ERROR 2!!!!!**********")
